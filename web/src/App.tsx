...
export default function App() {
  ...
  async function start(event:FormEvent) { event.preventDefault(); await action('start',async()=>{const result=await api<Login>(token,'/oauth/start','POST',{method,email:email||undefined,sso});setCode('');setLogin(result)}) }
  ...
  async function logout() { await action('logout',async()=>{await api(token,'/logout','POST');setConfirmLogout(false);setLogin(null);setData(await api<Status>(token,'/status'));setNotice('服务账号已退出。')}) }
